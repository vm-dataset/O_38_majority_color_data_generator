...
class TaskConfig(GenerationConfig):
    ...
    num_colors: int = Field(
        default=4,
        description="Number of different colors to use in the image"
    )
    
    num_shapes: int = Field(
        default=15,
        description="Total number of colored shapes to place"
    )
    
    shape_types: list[str] = Field(
        default_factory=lambda: ["circle", "rectangle", "ellipse"],
        description="Types of shapes to use"
    )
    
    min_shape_size: int = Field(
        default=30,
        description="Minimum shape size in pixels"
    )
    
    max_shape_size: int = Field(
        default=80,
        description="Maximum shape size in pixels"
    )
